...
//BAUDRATE of > 56000.0 disables baudrate simulation .
...
bool KPtyDevicePrivate::_k_canRead()
{
    ...
    if (::ioctl(q->masterFd(), PTY_BYTES_AVAILABLE, (char *) &available) != -1) {
        ...
        // Useless block braces except in Solaris
        {
          ...
          static float bytes_psec = BAUDRATE/9;

          if (bytes_psec > 0.0 && BAUDRATE <= 56000 ){
            NO_INTR(readBytes, read(q->masterFd(), ptr, 1));
            static float us = (1.0/bytes_psec)*1000000;
            QThread::usleep(us);
          } 
          else
          {
...
bool KPtyDevicePrivate::_k_canWrite()
{
    ...
    if (writeBuffer.isCtrlCPressed()){
        readBuffer.clear();
        ioctl(q->masterFd(), TCFLSH, 0); // flush receive
    }
